...
export const PinCard: React.FC<PinCardProps> = ({ pin, onPress, onDelete }) => {
  const badgeStyle = (() => {
    if (pin.category === 'Gittim') return { bg: 'rgba(16, 185, 129, 0.15)', text: '#10B981', border: 'rgba(16, 185, 129, 0.3)' };
    if (pin.category === 'İstek') return { bg: 'rgba(59, 130, 246, 0.15)', text: '#3B82F6', border: 'rgba(59, 130, 246, 0.3)' };
  ...
  return (
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.85}>
      {pin.imageUrl ? (
        <Image source={{ uri: pin.imageUrl }} style={styles.image} resizeMode="cover" />
      ) : (
      ...
      <View style={styles.content}>
        <View style={styles.headerRow}>
          <View style={styles.titleArea}>
            <Text style={styles.title} numberOfLines={1}>{pin.title}</Text>
          ...
          <View style={[styles.badge, { backgroundColor: badgeStyle.bg, borderColor: badgeStyle.border }]}>
            <Text style={[styles.badgeText, { color: badgeStyle.text }]}>{pin.category}</Text>
          </View>
        </View>
        ...
        <View style={styles.footer}>
          ...
          {pin.visitedDate && <Text style={styles.dateText}>{pin.visitedDate}</Text>}
